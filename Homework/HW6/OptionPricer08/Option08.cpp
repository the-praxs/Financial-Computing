	...
	double OptionCalculation::PriceByCRR(const BinomialTreeModel& Model, BinLattice<double>& PriceTree, BinLattice<double>& XTree, BinLattice<double>& YTree) {
		...
		for (int n = N - 1; n >= 0; n--) {
			for (int i = 0; i <= n; i++) {
				...
				X = (PriceTree.GetNode(n + 1, i + 1) - PriceTree.GetNode(n + 1, i)) / (Model.CalculateAssetPrice(n + 1, i + 1) - Model.CalculateAssetPrice(n + 1, i));
				Y = (PriceTree.GetNode(n + 1, i) - X * Model.CalculateAssetPrice(n + 1, i)) / pow((Model.GetR()), n + 1);
				XTree.SetNode(n, i, X);
				YTree.SetNode(n, i, Y);
			}
		}
